Vagrant.configure("2") do |config|
    ...
    config.vm.define "swarm-manager" do |manager|
        ...
        manager.vm.provision "shell", path: "other-steps.sh"
        manager.vm.provision "shell", path: "swarm-manager.sh"
    ...
    (2..3).each do |i|
        config.vm.define "worker0#{i}" do |worker|
          ...
          worker.vm.provision "shell", path: "other-steps.sh"
          worker.vm.provision "shell", path: "swarm-worker.sh"
